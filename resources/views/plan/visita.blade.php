...
<div class="page-wrap">
    ...
    <div class="d-flex align-items-center gap-3 flex-wrap mb-2">
        <h1 style="font-family:var(--font-display);font-size:28px;margin:0">Visita Iglesia Planner</h1>
            ...
            <div>
                <div class="upgrade-card mb-3">
                    <div style="position:relative">
                        <div style="color:rgba(255,255,255,0.7);font-size:12px;margin-bottom:4px">Route Progress</div>
                        <div style="font-family:var(--font-display);color:var(--gold);font-size:30px;font-weight:700;line-height:1">
                            <span id="visitedCount">0</span> of <span id="totalCountA">7</span>
                        </div>
                        <div style="color:rgba(255,255,255,0.8);font-size:13px;margin-top:2px">churches marked visited</div>
                        <div class="progress-track" style="margin-top:14px">
                            <div class="progress-fill" id="progressBar" style="width:0%"></div>
                        </div>
                        <div id="progressLabel" style="color:var(--gold);font-size:12px;font-weight:700;margin-top:6px">0% complete</div>
                    </div>
                </div>
                ...
                <div class="card card-body mb-3">
                    ...
                    <div class="d-flex align-items-center gap-4 mb-3">
                        <button type="button" onclick="GiyaVisita.step(-1)"
                                style="width:40px;height:40px;border-radius:12px;border:none;cursor:pointer;font-size:20px;font-weight:700;background:var(--gold-bg);color:var(--primary)">−</button>
                        <div class="text-center">
                            <div id="countValue" style="font-family:var(--font-display);font-size:30px;font-weight:700;color:var(--primary);line-height:1">7</div>
                            <div style="font-size:11px;color:var(--text-muted)">churches</div>
                        </div>
                        <button type="button" onclick="GiyaVisita.step(1)"
                                style="width:40px;height:40px;border-radius:12px;border:none;cursor:pointer;font-size:20px;font-weight:700;background:var(--primary);color:#fff">+</button>
                    </div>
                ...
                <div class="card card-body mb-3 d-none" id="addPicker">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <span style="font-size:14px;font-weight:700;color:var(--primary)">Add a Church</span>
                        <button type="button" class="route-btn" onclick="document.getElementById('addPicker').classList.add('d-none')">
                            <i class="bi bi-x" style="font-size:18px;color:var(--text-muted)"></i>
                        </button>
                    </div>
            ...
            <div>
                <div class="card card-body">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        ...
                        <span id="countLabel" style="font-size:12px;color:var(--text-muted)">7 churches</span>
                    </div>

                    <div id="churchList" style="display:flex;flex-direction:column;gap:10px"></div>

                    <button type="button" onclick="GiyaVisita.openPicker()"
                            style="width:100%;margin-top:12px;padding:12px;border-radius:13px;border:2px dashed rgba(215,169,74,.45);color:var(--primary);font-weight:600;font-size:13px;background:rgba(215,169,74,.05);cursor:pointer;font-family:var(--font-body)">
...
@push('head')
<style>
    .preset-btn { padding:6px 14px; border-radius:999px; font-size:12px; border:1.5px solid var(--border);
    ...
    function render() {
        ...
        document.getElementById('churchList').innerHTML = list.map(function (c, i) {
            ...
            const inner = c.visited
                ? '<i class="bi bi-check-lg" style="color:var(--gold);font-size:17px"></i>'
                : '<span style="font-size:12px;font-weight:700;color:' + (isCurrent ? 'var(--primary)' : 'var(--text-muted)') + '">' + (i + 1) + '</span>';
            ...
            return '<div class="vi-row ' + cls + '">' +
                ...
                '<span style="flex:1;min-width:0">' +
                  '<span style="display:block;font-size:14px;font-weight:600;color:var(--text)' + (c.visited ? ';text-decoration:line-through;opacity:.65' : '') + '">' + c.name + '</span>' +
                ...
                '<span style="display:flex;align-items:center;gap:8px;flex-shrink:0">' +
                  '<span style="display:flex;flex-direction:column;gap:2px">' +
                    '<button type="button" class="route-btn" onclick="GiyaVisita.move(' + i + ',-1)" style="color:' + (i === 0 ? '#D8C4BC' : 'var(--primary)') + ';font-size:12px">▲</button>' +
                    '<button type="button" class="route-btn" onclick="GiyaVisita.move(' + i + ',1)" style="color:' + (i === list.length - 1 ? '#D8C4BC' : 'var(--primary)') + ';font-size:12px">▼</button>' +
                  '</span>' +
                  '<button type="button" onclick="GiyaVisita.toggle(' + i + ')" style="padding:8px 13px;border-radius:10px;font-size:12px;font-weight:700;border:none;cursor:pointer;min-width:100px;font-family:var(--font-body);background:' + (c.visited ? 'var(--gold-bg)' : 'var(--primary)') + ';color:' + (c.visited ? 'var(--primary)' : '#fff') + '">' + (c.visited ? 'Visited' : 'Mark Visited') + '</button>' +
                  '<button type="button" onclick="GiyaVisita.drop(' + i + ')" style="width:28px;height:28px;border-radius:8px;background:rgba(212,24,61,.08);color:#D4183D;border:none;cursor:pointer;font-size:15px">×</button>' +
                '</span></div>';
        }).join('');
    }
    ...
    function refreshPicker() {
        ...
        box.innerHTML = available.length
            ? available.map(c =>
                ...
                '<span><span style="display:block;font-size:12px;font-weight:600;color:var(--text)">' + c.name + '</span>' +
                '<span style="display:block;font-size:10px;color:var(--text-muted)">' + c.location + '</span></span></button>').join('')
            : '<p style="font-size:12px;color:var(--text-muted);padding:8px;margin:0">Every destination is already in your route.</p>';
    }
